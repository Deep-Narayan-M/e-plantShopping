...
function ProductList({ onHomeClick }) {
  ...
  // Update addedToCart state whenever cartItems changes
  useEffect(() => {
    // Create a new object to track which items are in the cart
    const newAddedToCart = {};

    // Mark items that are in the cart
    for (let i = 0; i < cartItems.length; i++) {
      newAddedToCart[cartItems[i].name] = true;
    }
  ...
  // Pre-calculate total quantity once when cartItems changes
  ...
  const handleAddToCart = (product) => {
    // Extract the required properties from the product
